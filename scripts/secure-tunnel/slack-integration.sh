...
--target-openshift-cluster-login-string,    the target Openshift cluster Log in string, It used to create the Application mapping(port-forwarding) of the IBM Cloud Pak for Watson AIOps instance URL.
...
--https-proxy,                              if your AIOps cluster in a airgap environment and it cannot access the public network in the cluster,
                                            you can set a HTTPS_PROXY server for the Tunnel 
                                            then the Tunnel server with connect with the Tunnel connector use the HTTPS_PROXY server
-h, --help,                                 for help
...
echo "Check if the target Openshift cluster can be loged in ..."
login_public_cluster
...
echo
echo "Check if the AIOps cluster can be loged in ..."
login_aiops
...
if [ "${AIOPS_NAME}" == "" ]; then
    echo "Error: cannot found the aiops installation in this namespace, please install the aiops to this namespace first or change a namespace."
    exit 1
fi
...
if [ "${COUNT}" != "1" ]; then
    echo "the namespace ${PORT_FORWARDING_NAMESPACE} in the target openshift cluster not exist, will create it"
...
if [ "${COUNT}" == "1" ]; then
    echo "the docker image pull secret: ${PULL_SECRET_NAME} is exist, will delete and re-create it."
