...
class HashiCorpVault(object):
    ...
    def feed(self):
        ...
        if args.vault_url:
            self.vault_url = args.vault_url.rstrip("/")
        if args.token:
    ...
    def feed_creds_to_vault(self,creds_data):
        ...
        for cred in creds_data:
            ...
            cmd = ['curl', '-s' ,'-k' ,'-H', authorization ,'-H','Content-type: application/json','-d' , json.dumps(content) ,'-X' ,'POST' ,query_url ]
            # print (cmd)
            try:
    ...
    def create_policy_app_token(self):
        ...
        cmd = ['curl', '-s' ,'-k' ,'-H', authorization ,'-H','Content-type: application/json','-d' , json.dumps(payload) ,'-X' ,'POST' ,query_url ]
        try:
            proc = subprocess.Popen(cmd, stdout=subprocess.PIPE)
            out,err = proc.communicate()        
            response = json.loads(out)
    ...
    def create_policy_for_secret_engine(self):
        ...
        authorization = 'X-Vault-Token: ' + self.vault_root_token
        cmd = ['curl', '-s' ,'-k' ,'-H', authorization ,'-H','Content-type: application/json','-d' , json.dumps(vault_policy) ,'-X' ,'PUT' ,query_url ]
    ...
    def is_policy_present(self):
        ...
        query_url = self.vault_url + '/v1/sys/policy/' + self.policy_name
        authorization = 'X-Vault-Token: ' + self.vault_root_token
        cmd = ['curl', '-s' ,'-k' ,'-H', authorization ,'-H','Content-type: application/json', '-X' ,'GET' ,query_url ]
    ...
    def is_sample_secret_present(self):
        ...
        cmd = ['curl', '-s' ,'-k' ,'-H', authorization ,'-X' ,'GET' ,query_url ]
        # print (cmd)
        try:
            proc = subprocess.Popen(cmd, stdout=subprocess.PIPE)
            out,err = proc.communicate()
    ...
    def add_sample_secret(self):
        ...
        cmd = ['curl', '-s' ,'-k' ,'-H', authorization ,'-H','Content-type: application/json','-d' , json.dumps(ip_as_secret_name) ,'-X' ,'POST' ,query_url ]
